...
const ServiceManagement = () => {
  ...
  const [isCreateServiceModalOpen, setIsCreateServiceModalOpen] = useState(false); // Modal tạo service
  const [isTreatmentTypeModalOpen, setIsTreatmentTypeModalOpen] = useState(false); // Modal treatment stages
  ...
  const filteredServices = treatmentService.filter((service) =>
    service.name.toLowerCase().includes(searchQuery.toLowerCase())        // Filter theo tên service
  );
  ...
  const updateTreatmentStage = async (stageId, updatedFields) => {
    try {
      ...
      showNotification("Lỗi khi cập nhật liệu trình", "error");
    }
  };
                                    ...
                                    {editingStageId === stage.id ? (
                                      ...
                                      <div className="space-y-1">
                                        <input
                                          ...
                                          value={editName}
                                          onChange={(e) =>
                                            setEditName(e.target.value)         // Update edit name
                                          }
