...
const DoctorScheduleView = () => {
  ...
  const fetchManagerDashboardData = async () => {
    ...
    try {
      ...
      const statsRes = await treatmentService.getManagerWorkScheduleStatistics();
      if (statsRes?.data?.result) {
        setWorkStats(statsRes.data.result);
      } else {
        setWorkStats({ totalDoctorsToday: 0, totalPatientsToday: 0, completedPatientsToday: 0 });
      }
      ...
      notification.error({
        ...
        description: "Không thể tải thông tin bác sĩ và thống kê hôm nay. Vui lòng thử lại sau.",
        duration: 5
      });
    } finally {
      setLoading(false);
  ...
  const getShiftTag = (shift) => {
    if (shift === "FULL_DAY") return <Tag color="purple" icon={<CalendarOutlined />}>Cả ngày</Tag>;
    if (shift === "MORNING") return <Tag color="blue" icon={<ClockCircleOutlined />}>Sáng</Tag>;
    if (shift === "AFTERNOON") return <Tag color="orange" icon={<ClockCircleOutlined />}>Chiều</Tag>;
  ...
  const columns = [
    {
      ...
      render: (_, record) => (
        ...
        >
          ...
          />
          <div>
            <div className="font-bold text-lg text-gray-800">{record.doctorName}</div>
      ...
      render: (_, record) => (
        <div className="text-center">
          ...
          <div className="mt-2 text-xs text-gray-500">
            {record.shift === "FULL_DAY" ? "08:00 - 17:00" : 
             record.shift === "MORNING" ? "08:00 - 12:00" : "13:00 - 17:00"}
          </div>
        </div>
      ),
    },
      ...
      render: (_, record) => (
        <div className="text-center">
          ...
          <Progress
            ...
            size="small"
            strokeColor={getProgressColor(record.completedAppointments, record.totalAppointments)}
            showInfo={false}
          />
        </div>
      ),
    },
    {
      ...
      render: (phone) => (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <span style={{ fontWeight: 500, color: "#444", fontSize: 16 }}>{phone}</span>
        </div>
      ),
    },
  ];
    ...
    <div style={{ padding: "24px", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", minHeight: "100vh" }}>
      ...
      >
        <div className="text-center">
          ...
          <Text type="secondary" style={{ fontSize: "1.1rem" }}>
            Theo dõi và quản lý lịch làm việc của bác sĩ hôm nay
          </Text>
        </div>
      </Card>
      ...
      <Row gutter={24} style={{ marginBottom: 24 }}>
        <Col span={8}>
          ...
          >
            <Statistic
              title={<span style={{ color: "white", fontWeight: 600, fontSize: "1.1rem" }}>Tổng bác sĩ hôm nay</span>}
              value={workStats ? workStats.totalDoctorsToday : 0}
              prefix={<TeamOutlined style={{ color: "white", fontSize: "2rem" }} />}
        ...
        <Col span={8}>
          ...
          >
            <Statistic
              title={<span style={{ color: "white", fontWeight: 600, fontSize: "1.1rem" }}>Bệnh nhân đã khám</span>}
              value={workStats ? workStats.completedPatientsToday : 0}
              prefix={<CheckCircleOutlined style={{ color: "white", fontSize: "2rem" }} />}
              valueStyle={{ color: "white", fontSize: 36, fontWeight: "bold" }}
            />
          </Card>
        </Col>
        <Col span={8}>
          ...
          >
            <Statistic
              title={<span style={{ color: "white", fontWeight: 600, fontSize: "1.1rem" }}>Tổng bệnh nhân hôm nay</span>}
              value={workStats ? workStats.totalPatientsToday : 0}
              prefix={<UserOutlined style={{ color: "white", fontSize: "2rem" }} />}
              valueStyle={{ color: "white", fontSize: 36, fontWeight: "bold" }}
            />
          </Card>
        </Col>
      ...
      <Card 
        title={
          <div className="flex items-center">
            <CalendarOutlined style={{ fontSize: "1.5rem", color: "#1890ff", marginRight: 8 }} />
            <span style={{ fontSize: "1.3rem", fontWeight: "bold" }}>Lịch Làm Việc Bác Sĩ Hôm Nay</span>
          </div>
        } 
        className="shadow-lg"
        style={{
          ...
          locale={{
            emptyText: loading ? "" : (
              <div className="text-center py-8">
                <HeartOutlined style={{ fontSize: "3rem", color: "#d9d9d9", marginBottom: 16 }} />
                <div className="text-gray-500 text-lg">Không có dữ liệu bác sĩ hôm nay</div>
                <div className="text-gray-400">Vui lòng thử lại sau hoặc liên hệ quản trị viên</div>
              </div>
            )
          }}
      ...
      <Modal
        title={
          <div className="flex items-center">
            <UserOutlined style={{ fontSize: "1.5rem", color: "#1890ff", marginRight: 8 }} />
            <span style={{ fontSize: "1.3rem", fontWeight: "bold" }}>Thông tin chi tiết bác sĩ</span>
          </div>
        }
        ...
        {selectedDoctor ? (
          <div className="space-y-6">
            <div className="flex items-center space-x-6">
              ...
              />
              <div>
                <h3 className="text-2xl font-bold text-gray-800">{selectedDoctor.doctorName}</h3>
                ...
                <div className="space-y-4">
                  <div className="flex items-center p-3 bg-blue-50 rounded-lg">
                    ...
                    <div>
                      ...
                      <div className="text-gray-600">{selectedDoctor.phoneNumber}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center p-3 bg-green-50 rounded-lg">
                    ...
                    <div>
                      <div className="font-semibold text-gray-800">Ca làm việc</div>
              ...
              <Col span={12}>
                <div className="space-y-4">
                  <div className="flex items-center p-3 bg-orange-50 rounded-lg">
                    ...
                    <div>
                      <div className="font-semibold text-gray-800">Tổng lịch hẹn</div>
                      <div className="text-2xl font-bold text-orange-500">{selectedDoctor.totalAppointments}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center p-3 bg-purple-50 rounded-lg">
                    ...
                    <div>
                      ...
                      <div className="text-2xl font-bold text-purple-500">{selectedDoctor.completedAppointments}</div>
                    </div>
                  </div>
                </div>
              </Col>
          ...
          <div className="text-center py-8">
            <UserOutlined style={{ fontSize: "3rem", color: "#d9d9d9", marginBottom: 16 }} />
