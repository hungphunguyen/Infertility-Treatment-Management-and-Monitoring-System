...
import { 
  ...
  Timeline, Modal, Descriptions, Spin, message, Button, DatePicker, Select, Form, Input, Alert 
} from 'antd';
import { 
  ExperimentOutlined, 
  CheckCircleOutlined, 
...
const MyServices = () => {
  ...
  const handleOpenChangeModal = async (step) => {
    ...
    try {
      const res = await treatmentService.getCustomerAppointments(selectedService.customerId);
      if (res?.data?.result) {
        ...
        if (found) {
          changeForm.setFieldsValue({
            requestedDate: found.appointmentDate ? dayjs(found.appointmentDate) : null,
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <Timeline>
                {selectedService.treatmentSteps?.map((step, index) => {
                  ...
                  const s = statusMap[step.status] || { color: 'default', text: step.status };
                  return (
                    <Timeline.Item 
                      key={index}
                      color={s.color}
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <Button size="small" type="primary" onClick={() => handleOpenChangeModal(step)}>
        ...
        {changeLoading ? <Spin /> : (
          changeAppointment ? (
            <Form form={changeForm} layout="vertical">
              <Form.Item label="Ngày hẹn mới" name="requestedDate" rules={[{ required: true, message: 'Chọn ngày mới' }]}> 
                <DatePicker style={{ width: '100%' }} />
              </Form.Item>
              <Form.Item label="Ca khám mới" name="requestedShift" rules={[{ required: true, message: 'Chọn ca khám' }]}> 
            ...
            <Alert type="warning" message="Không tìm thấy lịch hẹn tương ứng cho bước này!" />
          )
        )}
      </Modal>
    </div>
